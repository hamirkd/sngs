...
<page orientation="paysage" format="A4" backtop="30mm" backbottom="10mm" backleft="10mm" backright="10mm">
    ...
    if ($_SESSION['userMag'] > 0)/**/ {
        ...
        if (!empty($search['d']) && empty($search['f']))
            $conddate.=" AND date(ap.date_appro)='" . isoToMysqldate($search['d']) . "'";

        if (!empty($search['f']))
            $conddate.=" AND date(ap.date_appro) between '" . isoToMysqldate($search['d']) . "' 
                AND '" . isoToMysqldate($search['f']) . "'";

        $querydat = "SELECT DISTINCT(date(ap.date_appro)) FROM t_approvisionnement ap
            INNER JOIN t_approvisionnement_article apa on ap.id_appro=apa.appro_appro_art
            WHERE apa.mag_appro_art=" . intval($_SESSION['userMag']) . "
                     AND $conddate";
        $rdate = $Mysqli->query($querydat);
        $rowdate = $rdate->fetch_assoc();
echo $querydat;
        while ($rowdate) {
            ?>
            <div style="font-size: 14pt;text-transform: capitalize; font-weight: bold; padding:5pt;" align="left">
                <?php echo $rowdate['date_appro']; ?>    
            </div> 
            <?php
            ...
            if ($r->num_rows > 0) {
                ...
                <?php
            }/* fin du nume_rows sup a zero de la liste des articles appro */
        }/* fin du parcours dela premiere date */
