...
<page backtop="30mm" backbottom="10mm" backleft="20mm" backright="20mm">
    ...
    <div style="font-size: 20px;text-transform: capitalize; 
        ...
        <u>Liste des reglements clients
        </u>   
    </div> 
    <br>
    ...
    $condmag = "";
        ...
        $query = "SELECT count(c.ref_paye_crce_clnt) as numverse, date(c.date_crce_clnt) as date_crce_clnt,
   ...
   /* if (!empty($search['mg']))
    ...
    $query.=" GROUP BY c.ref_paye_crce_clnt ORDER BY c.date_crce_clnt DESC";
        ...
        <table class="data" cellspacing="0" cellpadding="5mm" style="width: 100%; border: solid 1px black;text-align: center; font-size: 10pt;">
            <tr style="">
                <th style="width: 15%;text-align: left;border:1px solid black;">DATE</th>
                 <th style="width: 28%;text-align: left;border:1px solid black;">CLIENT</th>
                <th style="width: 14%;text-align: left;border:1px solid black;">MONTANT</th>
                <th style="width: 30%;text-align: left;border:1px solid black;">REF</th>
            ...
            while ($row = $r->fetch_assoc()) {
                $total +=$row['mnt_glob_crce_clnt'];
                ?>
                <tr >
                    <td style="width: 15%;text-align: left"><?php echo text_reduit($row['date_crce_clnt'],10); ?></td> 
                     <td style="width: 28%;text-align: left"><?php echo ucwords(strtolower($row['nom_clt'])); ?></td>
                    <td style="width: 14%; text-align: right"><?php echo number_format($row['mnt_glob_crce_clnt'], 0, ',', ' '); ?></td>
                     <td style="width: 30%; text-align: right"><?php echo text_reduit($row['ref_crce_clnt'],50); ?></td>
