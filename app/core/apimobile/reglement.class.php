    ...
    public function getCreancesgClients() {
        ...
        $c_us = " 1=1 ";
         if (isset($_SESSION['mode_clnt_uniq'])&&$_SESSION['mode_clnt_uniq'] == 0)
            $c_us = $c_us." AND c.user_clt in (SELECT id_user from t_user where mag_user=" . $_SESSION['userMag'] . ")";
        ...
        $query = "SELECT id_clt,code_clt,nom_clt,(SELECT SUM(crdt_fact)-SUM(som_verse_crdt) FROM t_facture_vente f
                    WHERE f.bl_fact_crdt=1 AND f.bl_fact_grt=0 AND f.sup_fact=0
                    $condMag AND f.bl_crdt_regle=0 AND f.crdt_fact>0 AND (f.crdt_fact-f.som_verse_crdt)>0 
                    AND f.clnt_fact = c.id_clt 
                    GROUP BY f.clnt_fact) as mnt_reste  FROM t_client c 
                    WHERE  $c_us HAVING(mnt_reste>0) ORDER BY c.nom_clt ASC limit $limit_debut,$taille ";
