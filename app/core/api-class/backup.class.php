 ...
 * Sauvegarde MySQL
 */
class BackupMySQL extends model {
    ...
    /**
     * Structure proprietaire du fichier
     * @var string
     */

    /**
     * Objets a ne pas sauvegarder
     * @var string
     */
    protected $excludeElement = array("t_audit",
        "t_user_profil",
        "t_droit", "t_log",
        "t_user_droit",
        "t_user_menu",
        "t_user_preferences",
        "v_etat_alertes",
        "v_etat_periode_appro",
        "v_etat_periode_vente",
        "v_etat_stock",
        "v_etat_ventes",
        "v_valeur_stock");
    ...
    public function __construct($options = array()) {
        ...
        // Controle du fichier
        $this->nom_fichier = $this->proprietaire . "-" . $nom_fichier . date('Ymd-His') . '.sql.gz';
    ...
    protected function sauvegarder() {
        ...
        // Liste les tables
        $result_tables = $this->mysqli->query('SHOW TABLE STATUS');
        if ($result_tables && $result_tables->num_rows) {
            while ($obj_table = $result_tables->fetch_object()) {
                ...
                if (!in_array($obj_table->{'Name'}, $this->excludeElement)) {
                    // DROP ...
                    $sql = "\n\n";
                    $sql .= 'DROP TABLE IF EXISTS `' . $obj_table->{'Name'} . '`' . ";\n";

                    // CREATE ...
                    $result_create = $this->mysqli->query('SHOW CREATE TABLE `' . $obj_table->{'Name'} . '`');
                    ...
                    // INSERT ...
                    $result_insert = $this->mysqli->query('SELECT * FROM `' . $obj_table->{'Name'} . '`');
        ...
        $mailer = new Mail();
        $sujet = $this->proprietaire . "-SAUV-BD-" . date('Ymd-His');
        $message = "Sauvegarde datas <br/> Cordialement ! <br/> Tongo Technologie Systems..";
        $addresdereponse = "tongotechnologie@gmail.com";
        $nomdereponse = "TNG-TECHNO-SYSTEMS";
        $repondresur = "tongotechnologie@gmail.com";
        $enPj = htmlspecialchars($this->nom_fichier);
        $mailer->sendEmail("tongotechnologie@gmail.com", $sujet, $message, "", $addresdereponse, $nomdereponse, $repondresur, $enPj);
    }
