...
class stockController extends model {
    ...
    public function updateStock() {
        ...
        if($prec_qte_stk==$q){
            ...
            $query = 
            "UPDATE t_stock SET prec_qte_stk=qte_stk,
    ...
    public function setAdresseSotck() {
        if ($this->get_request_method() != "POST") {
            $this->response('', 406);
        }
        $approvisionnement = $_POST;
        ...
        $address = $this->esc($approvisionnement['stock']['adr_stk']);

        if ($address == "")
            $address = "-";

        if (strpos(strtolower($address), "<br>") == FALSE)
            $address = $address;
        else
            $address = substr(strtolower($address), 0, -4);



        $query = "UPDATE t_stock SET adr_stk='$address' WHERE id_stk=$id";
        $response = array();
...
p.prix_mini_art,p.prix_max_art,p.prix_gros_art                
from t_stock s
  ...
  WHERE 1=1";
    ...
    public function queryEtatStock() {
        ...
        if ($_SESSION['userMag'] != 0)
            $query = "SELECT *,p.prix_mini_art,p.prix_max_art,p.prix_gros_art FROM v_etat_stock 
                left join (select * from t_prix_article GROUP BY art_prix_art DESC) p on v_etat_stock.art_stk=p.art_prix_art
                WHERE (nom_art like '%$qry%' OR ref_art like '%$qry%' OR nom_cat like '%$qry%') AND  mag_stk=" . intval($_SESSION['userMag']);
        else
            $query = "SELECT *,p.prix_mini_art,p.prix_max_art,p.prix_gros_art FROM v_etat_stock 
                left join (select * from t_prix_article GROUP BY art_prix_art DESC) p on v_etat_stock.art_stk=p.art_prix_art
    ...
    public function etatStocklm() {
        ...
        if ($_SESSION['userMag'] != 0)
            $query = "select s.*,a.nom_art,a.ref_art,a.code_art,ca.id_cat,ca.nom_cat,a.seuil_art,a.unite_art,a.cat_art,m.nom_mag,
p.prix_mini_art,p.prix_max_art,p.prix_gros_art                 
from t_stock s
   inner join t_magasin m on s.mag_stk=m.id_mag
  inner join t_article a on s.art_stk=a.id_art
  inner join t_categorie_article ca on a.cat_art=ca.id_cat
  left join (select * from t_prix_article WHERE a_jour=0 GROUP BY art_prix_art DESC) p on s.art_stk=p.art_prix_art
  WHERE s.mag_stk=" . intval($_SESSION['userMag']);
        else
            $query = "select s.*,a.nom_art,a.ref_art,a.code_art,ca.id_cat,ca.nom_cat,a.seuil_art,a.unite_art,a.cat_art,m.nom_mag,
   p.prix_mini_art,p.prix_max_art,p.prix_gros_art              
from t_stock s
  ...
  WHERE 1=1";
        ...
        $query .= " Order by m.nom_mag,ca.nom_cat,a.nom_art limit 50";
...
from t_stock s
  ...
  WHERE 1=1";
        ...
        $query .= " Order by m.nom_mag,ca.nom_cat,a.nom_art limit 50 offset $offset";
...
WHERE s.qte_stk <= a.seuil_art";
    ...
    public function getStock() {
        ...
        if (!empty($this->_request['id_art'])) {
            ...
            $id_mag = (!empty($this->_request['id_mag'])) ? intval($this->_request['id_mag']) : intval($_SESSION['userMag']);
            ...
            $query2 = "SELECT sum(qte_stk) as qteg FROM t_stock  WHERE art_stk=$id_art LIMIT 1";
    ...
    public function getArticleEntrees() {
        ...
        $id = (int) $_GET['id'];
        $query = "SELECT asort.id_appro_art,asort.qte_appro_art,asort.prix_appro_art,asort.user_appro_art,asort.date_appro_art,
           a.nom_art,a.id_art  
